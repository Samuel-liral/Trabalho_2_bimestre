...
void inicializarTabuleiro(Tabuleiro *t){
	...
	do{
		...
		printf("Digite a quantidade de linhas (8 MIN) (10 MAX): ");
		scanf("%d", &linhas);
		printf("Digite a quantidade de colunas (8 MIN) (10 MAX): ");
		scanf("%d", &colunas);
		system("cls");
		while(getchar() != '\n');
		
		verificar = 1;
	} while(!(linhas >= 8 && linhas <= 10 && colunas >= 8 && colunas <= 10));
	verificar = 0;
...
char* traduzir(int num){
	switch(num){
		case 1: return "Insano (80%)";
		case 2: return "Pesadelo (90%)";
		default: NULL;
	}
}
