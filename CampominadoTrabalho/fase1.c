...
void inicializarTabuleiro(Tabuleiro *t){
	...
	do{
		...
		printf("Digite a quantidade de linhas (3 MIN) (10 MAX): ");
		scanf("%d", &linhas);
		printf("Digite a quantidade de colunas (3 MIN) (10 MAX): ");
		scanf("%d", &colunas);
		system("cls");
		while(getchar() != '\n');
		
		verificar = 1;
	} while(!(linhas >= 3 && linhas <= 10 && colunas >= 3 && colunas <= 10));
	verificar = 0;
...
char* traduzir(int num){
	switch(num){
		case 1: return "Extremamente Facil(5%)";
		case 2: return "Muito Facil (10%)";
		default: NULL;
	}
}
...
void colocarMinas(Tabuleiro *t, int *minas){
    ...
    do{
	    ...
	    printf("[1] Extremamente Facil(5%%)\n");
	    printf("[2] Muito Facil (10%%)\n");
