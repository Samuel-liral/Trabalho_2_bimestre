...
void jogar(Tabuleiro *tabuleiro, const char* jogadorNome){
    ...
    int i, j;
    for(i = 0 ; i < colunas ; i++){
    	free(tabuleiro->visivel[i]);
	}
	free(tabuleiro->visivel);
	
	for(i = 0 ; i < colunas ; i++){
    	free(tabuleiro->minas[i]);
	}
	free(tabuleiro->minas);
...
void inicializarTabuleiro(Tabuleiro *t){
	...
	do{
		...
		printf("Digite a quantidade de linhas (5 MIN) (10 MAX): ");
		scanf("%d", &linhas);
		printf("Digite a quantidade de colunas (5 MIN) (10 MAX): ");
		scanf("%d", &colunas);
		system("cls");
		while(getchar() != '\n');
		
		verificar = 1;
	} while(!(linhas >= 5 && linhas <= 10 && colunas >= 5 && colunas <= 10));
	verificar = 0;
...
char* traduzir(int num){
	switch(num){
		case 1: return "Dificil (30%)";
		case 2: return "Muito Dificil (40%)";
		default: NULL;
	}
}
